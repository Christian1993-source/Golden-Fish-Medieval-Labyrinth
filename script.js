(function () {
  ...
  const touchButtons = [touchUpBtn, touchDownBtn, touchLeftBtn, touchRightBtn].filter(Boolean);
  ...
  function formatLevelWithDifficulty(level) {
    return `${cleanLevelName(level.name)} [${level.difficulty}]`;
  }
  ...
  function bindTouchDirectionButton(button, key) {
    if (!button) {
      return;
    }
  ...
  function updateSoundButtonUi() {
    const soundButtons = [soundToggleBtn, mobileSoundBtn].filter(Boolean);
  ...
  function updateLevelInfoPreview(index) {
    ...
    if (levelNameEl) {
      levelNameEl.textContent = formatLevelWithDifficulty(level);
    }
    if (levelSizeEl) {
  ...
  function populateLevelSelect() {
    ...
    if (mobileLevelSelect) {
      mobileLevelSelect.innerHTML = '';
    }

    window.LEVELS.forEach(function (level, index) {
      const option = document.createElement('option');
      option.value = String(index);
      option.textContent = `Stage ${index + 1} - ${formatLevelWithDifficulty(level)}`;
      levelSelect.appendChild(option);

      if (mobileLevelSelect) {
        const mobileOption = document.createElement('option');
        mobileOption.value = option.value;
        mobileOption.textContent = option.textContent;
        mobileLevelSelect.appendChild(mobileOption);
      }
    });
  }
  ...
  function loadLevel(index, options) {
    ...
    if (mobileLevelSelect) {
      mobileLevelSelect.value = String(state.levelIndex);
    }
  ...
  function setupInput() {
    function handleLevelSelection(selectedIndex) {
    ...
    levelSelect.addEventListener('change', function () {
      ...
      handleLevelSelection(selectedIndex);
    });

    if (mobileLevelSelect) {
      mobileLevelSelect.addEventListener('change', function () {
        const selectedIndex = Number(mobileLevelSelect.value);
        handleLevelSelection(selectedIndex);
      });
    }
    ...
    bindTouchDirectionButton(touchUpBtn, 'ArrowUp');
    bindTouchDirectionButton(touchDownBtn, 'ArrowDown');
    bindTouchDirectionButton(touchLeftBtn, 'ArrowLeft');
    bindTouchDirectionButton(touchRightBtn, 'ArrowRight');
  }
